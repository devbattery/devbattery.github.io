...
function App() {
    ...
    useEffect(() => {
        ...
        // ... (SSE 설정 로직)
        if (isLoggedIn) { // 로그인 되었을 때만 SSE 연결 시도
            setupSSE();
        }
        // ...
        return () => {
                        ...
                        <div className="nav-right-logged-in">
                            ...
                            <NavLink to="/mypage">
                                {userInfo.profileImgUrl ? (
                                    <img src={userInfo.profileImgUrl} alt="프로필" className="nav-mypage-img" />
                                ) : (
...
export default defineConfig(({mode}) => {
    ...
    if (isDev) {
        ...
        proxyConfig['/oauth2'] = { // 예: /oauth2/authorization/google
            target: `${env.VITE_BASE_URL_FOR_CONF}`,
            changeOrigin: true,
            secure: false,
        };
        proxyConfig['/token'] = { // OAuthCallback.jsx에서 사용하는 /token 엔드포인트
            target: `${env.VITE_BASE_URL_FOR_CONF}`,
            changeOrigin: true,
            secure: false,
        };
        proxyConfig['/refresh'] = { // axiosInstance.js에서 사용하는 /refresh 엔드포인트
            target: `${env.VITE_BASE_URL_FOR_CONF}`,
            changeOrigin: true,
            secure: false,
        };
